...
main_application.title('HV text editor')
...
def voice_type(event=None):
    ...
    try:
        with sr.Microphone() as source2:
            ...
            MyText1 = ". "+ MyText.capitalize()
            text_editor.insert(tk.END, MyText1)
    except sr.RequestError as e:
        print("Could not request results; {0}".format(e))
    except sr.UnknownValueError:
        print("Unknown error occured") 
tools.add_command(label='Voice Typing', command=voice_type)    
            
def textspeech(event=None):
    mytext = text_editor.clipboard_get()
    language="en"
    myobj= gTTS(text=mytext, lang=language, slow=False)
    myobj.save("welcome.mp3")
    os.system("welcome.mp3")
    
tools.add_command(label="Text to Speech", command=textspeech)
...
view.add_checkbutton(label='Toolbar', onvalue=True, offvalue=0,variable= show_toolbar, command=hide_toolbar)
view.add_checkbutton(label='Statusbar',onvalue=1, offvalue=False,variable= show_statusbar, command=hide_statusbar)
...
count = 0
for i in color_dict:
    color_menu.add_radiobutton(label=i, image =color_icons[count],variable = theme_choice,compound=tk.LEFT, command = change_theme)
    count+=1
